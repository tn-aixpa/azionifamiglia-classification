...
def save_splits(X, masks, y, directory, mlb, data_path, seeds):
    ...
    for seed in seeds:
        ...
        stratifier = IterativeStratification(
            n_splits=2, order=2, sample_distribution_per_fold=[0.2, 0.8]
        )
        ...
        # Save the splits
        np.save(
            os.path.join(data_path, directory, f"split_{seed}", "train_X.npy"), train_X
        )
        np.save(
            os.path.join(data_path, directory, f"split_{seed}", "train_mask.npy"),
            train_mask,
        )
        np.save(
            os.path.join(data_path, directory, f"split_{seed}", "train_y.npy"), train_y
        )

        np.save(os.path.join(data_path, directory, f"split_{seed}", "dev_X.npy"), dev_X)
        np.save(
            os.path.join(data_path, directory, f"split_{seed}", "dev_mask.npy"),
            dev_mask,
        )
        np.save(os.path.join(data_path, directory, f"split_{seed}", "dev_y.npy"), dev_y)

        np.save(
            os.path.join(data_path, directory, f"split_{seed}", "test_X.npy"), test_X
        )
        np.save(
            os.path.join(data_path, directory, f"split_{seed}", "test_mask.npy"),
            test_mask,
        )
        np.save(
            os.path.join(data_path, directory, f"split_{seed}", "test_y.npy"), test_y
        )

        # Save the counts of each label, useful for weighted loss
        sample_labs = mlb.inverse_transform(train_y)
        labs_count = {
            "total_samples": len(sample_labs),
            "labels": {label: 0 for label in mlb.classes_},
        }
        ...
        with open(
            os.path.join(
                data_path, directory, f"split_{seed}", "train_labs_count.json"
            ),
            "w",
        ) as fp:
...
def process_year(
    ...
    with gzip.open(path, "rt", encoding="utf-8") as file:
        ...
        if get_doc_ids:
            ...
            for doc in data:
                print(
                    f"{datetime.now().replace(microsecond=0)} - {j}/{len(data)}",
                    end="\r",
                )
                j += 1
            ...
            for doc in data:
                ...
                if add_mt_do:
                    ...
                    labels = (
                        set(data[doc]["eurovoc_classifiers"])
                        if "eurovoc_classifiers" in data[doc]
                        else set(data[doc]["eurovoc"])
                    )
                    to_add = set()
                    for label in labels:
                    ...
                    labels = (
                        data[doc]["eurovoc_classifiers"]
                        if "eurovoc_classifiers" in data[doc]
                        else data[doc]["eurovoc"]
                    )
                ...
                if title_only:
                    ...
                    if summarized:
                        ...
                        phrase_importance = sorted(
                            phrase_importance, key=lambda x: x[1], reverse=True
                        )
                        ...
                        if (
                            len(
                                " ".join(
                                    [
                                        full_text[phrase[0]]
                                        for phrase in phrase_importance
                                    ]
                                ).split()
                            )
                            > max_length
                        ...
                        phrase_importance = sorted(
                            phrase_importance, key=lambda x: x[0]
                        )
                        text += " ".join(
                            [full_text[phrase[0]] for phrase in phrase_importance]
                        )
                    else:
                        text += (
                            data[doc]["full_text"]
                            if "full_text" in data[doc]
                            else data[doc]["text"]
                        )
...
def process_datasets(
    ...
    if years == "all":
        years = [
            year
            for year in os.listdir(os.path.join(data_path, directory))
        ...
        if any(are_any_summarized):
            sum_type = files_in_directory[are_any_summarized.index(True)].split("_", 1)[
                1
            ]
    ...
    with gzip.open(
        os.path.join(data_path, directory, years[0]), "rt", encoding="utf-8"
    ) as file:
        data = json.load(file)
    ...
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
    X = pad_sequence(
        list_inputs, batch_first=True, padding_value=tokenizer.pad_token_id
    ).numpy()
    masks = pad_sequence(list_masks, batch_first=True, padding_value=0).numpy()

    # Save the MultiLabelBinarizer.
    with open(
        os.path.join(data_path, directory, "mlb_encoder.pickle"), "wb"
    ) as pickle_fp:
